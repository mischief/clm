...
openai_build_request(const struct clm_llm *llm, cJSON *messages, cJSON *tools, bool stream)
{
	...
	/* Disable parallel tool calls -- a tool host that can only process one
	 * action at a time (e.g. a game bridge advancing one action per game
	 * turn) deadlocks on parallel dispatch, and serial calls keep tool
	 * ordering deterministic everywhere else. */
