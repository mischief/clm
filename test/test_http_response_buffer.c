...
test_exact_limit(void)
{
	...
	CHECK(http_response_buffer_write(&buf, "abc", 1, 3, true, &written) ==
	        HTTP_RESPONSE_BUFFER_OK,
	    "first buffered write succeeds");
	CHECK(written == 3, "first buffered write reports its size");
	CHECK(http_response_buffer_write(&buf, "defgh", 5, 1, true, &written) ==
	        HTTP_RESPONSE_BUFFER_OK,
...
test_over_limit(void)
{
	...
	CHECK(http_response_buffer_write(&buf, "abcd", 1, 4, true, &written) ==
	        HTTP_RESPONSE_BUFFER_OK,
	    "limit-sized response succeeds");
	CHECK(http_response_buffer_write(&buf, "e", 1, 1, true, &written) ==
...
test_streaming_does_not_buffer(void)
{
	...
	CHECK(http_response_buffer_write(&buf, "abcd", 2, 2, false, &written) ==
	        HTTP_RESPONSE_BUFFER_OK,
	...
	CHECK(http_response_buffer_write(&buf, "e", 1, 1, false, &written) ==
	        HTTP_RESPONSE_BUFFER_TOO_LARGE,
...
test_received_overflow(void)
{
	...
	CHECK(http_response_buffer_write(&buf, "x", 1, 1, false, &written) ==
...
test_allocation_size_overflow(void)
{
	...
	CHECK(http_response_buffer_write(&buf, "x", 1, 1, true, &written) ==
	        HTTP_RESPONSE_BUFFER_OVERFLOW,
